-- FYBLIC V1080 — moteur unifié Normal + Story + Boutique.
...
create index if not exists fyblic_publication_jobs_group_v1080
...
create or replace function public.fyblic_worker_claim_job_v1080(p_worker_id text,p_lease_seconds integer default 1800)
...
create or replace function public.fyblic_worker_progress_v1080(
...
create or replace function public.fyblic_worker_heartbeat_v1080(
...
create or replace function public.fyblic_worker_finalize_album_group_v1080(
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended('fyblic-v1080-album:'||p_user_id::text||':'||p_group_id,0));
  ...
  loop
    ...
    v_post:=jsonb_build_object(
      ...
      'image_crop',coalesce(v_payload->'imageCrop','{}'::jsonb)||jsonb_build_object('adaptive',jsonb_build_object('v',3,'mode','connection','variants',v_variants)),
...
create or replace function public.fyblic_worker_publish_primary_v1080(
...
begin
  ...
  if v_job.publication_type not in ('normal','story','boutique','album') then raise exception 'TYPE_NOT_ENABLED_V1080'; end if;
  ...
  if v_job.publication_type='album' then
    perform public.fyblic_worker_finalize_album_group_v1080(v_job.publication_group_id,v_job.user_id);
  end if;
...
create or replace function public.fyblic_worker_complete_variants_v1080(
  p_job_id uuid,p_worker_id text,p_result jsonb
)
returns jsonb language plpgsql security definer set search_path=public as $$
declare
...
begin
  ...
  if v_job.publication_type in ('normal','album') then
    v_crop:=coalesce(v_job.payload->'imageCrop','{}'::jsonb)||jsonb_build_object('adaptive',jsonb_build_object('v',3,'mode','connection','variants',coalesce(p_result->'variants','{}'::jsonb)));
...
create or replace function public.fyblic_publication_schema_status_v1080()
...
select jsonb_build_object(
  ...
  'normal',to_regprocedure('public.fyblic_worker_publish_primary_v1080(uuid,text,jsonb)') is not null,
  'story',to_regprocedure('public.fyblic_insert_story_json_v1069(jsonb)') is not null,
  'boutique',to_regprocedure('public.fyblic_worker_complete_variants_v1080(uuid,text,jsonb)') is not null,
  'album',to_regprocedure('public.fyblic_worker_publish_primary_v1080(uuid,text,jsonb)') is not null,
  'claim_optimizing',to_regprocedure('public.fyblic_worker_claim_job_v1080(text,integer)') is not null,
  'heartbeat',to_regprocedure('public.fyblic_worker_heartbeat_v1080(uuid,text,integer)') is not null,
  ...
  'group_finalize',to_regprocedure('public.fyblic_worker_finalize_album_group_v1080(text,uuid)') is not null,
...
values ('publication_pipeline',1080,'{"normal":true,"story":true,"boutique":true,"album":true,"unified_engine":true,"group_assets":true,"group_finalize":true,"claim_optimizing":true,"heartbeat":true}'::jsonb,now(),now())
on conflict(component) do update set version=excluded.version,capabilities=excluded.capabilities,updated_at=now();

revoke all on function public.fyblic_worker_claim_job_v1080(text,integer) from public,anon,authenticated;
revoke all on function public.fyblic_worker_progress_v1080(uuid,text,integer,text,integer) from public,anon,authenticated;
revoke all on function public.fyblic_worker_heartbeat_v1080(uuid,text,integer) from public,anon,authenticated;
revoke all on function public.fyblic_worker_publish_primary_v1080(uuid,text,jsonb) from public,anon,authenticated;
revoke all on function public.fyblic_worker_finalize_album_group_v1080(text,uuid) from public,anon,authenticated;
revoke all on function public.fyblic_worker_complete_variants_v1080(uuid,text,jsonb) from public,anon,authenticated;
revoke all on function public.fyblic_publication_schema_status_v1080() from public,anon,authenticated;

grant execute on function public.fyblic_worker_claim_job_v1080(text,integer) to service_role;
grant execute on function public.fyblic_worker_progress_v1080(uuid,text,integer,text,integer) to service_role;
grant execute on function public.fyblic_worker_heartbeat_v1080(uuid,text,integer) to service_role;
grant execute on function public.fyblic_worker_publish_primary_v1080(uuid,text,jsonb) to service_role;
grant execute on function public.fyblic_worker_finalize_album_group_v1080(text,uuid) to service_role;
grant execute on function public.fyblic_worker_complete_variants_v1080(uuid,text,jsonb) to service_role;
grant execute on function public.fyblic_publication_schema_status_v1080() to service_role;

commit;

select public.fyblic_publication_schema_status_v1080() as fyblic_v1080_status;
