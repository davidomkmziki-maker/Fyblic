/* Fyblic V1081 — moteur unique de publication : Normal + Story + Boutique.
...
(function(){
  ...
  var REQUIRED_SCHEMA_VERSION=1081;
  var REQUIRED_PROTOCOL_VERSION=1081;
  ...
  function terminalStatus(status){status=clean(status);return status==='failed'||status==='canceled'||status==='published';}
  ...
  async function available(force){
    ...
    try{
      ...
      var ok=!!(r.body&&r.body.ok&&p.enabled&&p.ready&&p.schemaReady&&Number(p.schemaVersion||0)>=REQUIRED_SCHEMA_VERSION&&Number(p.protocolVersion||0)>=REQUIRED_PROTOCOL_VERSION&&p.uploadMode==='direct-tus-binary'&&caps.normal===true&&caps.story===true&&caps.boutique===true&&caps.album===true&&caps.groupAssets===true&&caps.groupFinalize===true&&caps.postMediaSafe===true);
  ...
  function readinessMessage(){var d=healthCache.detail||{},e=clean(d.schemaError);if(Number(d.schemaVersion||0)<REQUIRED_SCHEMA_VERSION||/1081|migration|schema|schéma/i.test(e))return 'Mise à jour SQL V1081 requise avant de publier.';if(Number(d.protocolVersion||0)<REQUIRED_PROTOCOL_VERSION||d.uploadMode!=='direct-tus-binary')return 'Mise à jour du worker V1081 requise avant de publier.';if(e)return publicError(e);return 'Système de publication momentanément indisponible.';}
  ...
  async function uploadDirect(file,created,currentAuth,onProgress){
    var upload=created&&created.upload;if(!upload||upload.mode!=='direct-binary'||!/^https:\/\//i.test(clean(upload.url)))throw new Error('Session d’envoi direct V1081 absente');
  ...
  async function submitMany(files,options){
    ...
    if(!(await available(true)))return {used:false,reason:'engine-v1081-unavailable'};
    var a=await auth();
    ...
    function emit(index,job){states[index]=mergeJobState(states[index],job||{});var d=aggregate(group,states);dispatch(d);if(typeof options.onProgress==='function')options.onProgress(d,states.slice());}
  ...
  var api={version:'1081.0',requiredSchemaVersion:REQUIRED_SCHEMA_VERSION,requiredProtocolVersion:REQUIRED_PROTOCOL_VERSION,uploadMode:'direct-tus-binary',available:available,readinessMessage:readinessMessage,submit:submit,submitMany:submitMany,status:status,watch:watch,resumeTracking:resumeTracking,cancel:cancel,forget:forget};window.FyblicPublicationEngineV1081=api;window.FyblicPublicationEngineV1080=api;
