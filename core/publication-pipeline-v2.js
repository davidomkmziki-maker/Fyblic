(function(){
  ...
  function remember(job){if(!job||!job.id)return;var list=readJobs().filter(function(x){return x&&x.id!==job.id;});list.unshift({id:job.id,postId:job.post_id||job.postId||'',status:job.status||'uploading',progress:Number(job.progress||0),stage:job.stage||'',createdAt:job.created_at||new Date().toISOString()});writeJobs(list);}
  function forget(id){writeJobs(readJobs().filter(function(x){return x&&x.id!==id;}));}
  function event(job){
    if(!job)return;
    remember(job);
    var detail={jobId:job.id,postId:job.post_id||job.postId||'',status:job.status||'',progress:Number(job.progress||0),stage:job.stage||'',error:job.error_message||'',result:job.result||null};
    ...
    if(['published','failed','canceled'].indexOf(detail.status)>=0){setTimeout(function(){if(detail.status==='published'||detail.status==='canceled')forget(job.id);},detail.status==='published'?8000:1000);}
  }
  ...
  async function submit(file,options){
    ...
    if(created.status==='published'){return {used:true,job:created,completion:Promise.resolve(created)};}
    var offset=Number(created.uploaded_bytes||0),retries=0;
    while(offset<file.size){
      var end=Math.min(offset+CHUNK_BYTES,file.size),blob=file.slice(offset,end),data=await chunkBase64(blob);
      try{
  ...
  async function resumeTracking(){
    var jobs=readJobs().filter(function(j){return j&&j.id&&['published','canceled'].indexOf(j.status)<0;});if(!jobs.length)return;
  ...
  window.FyblicPublicationPipelineV2={version:'1063.1',available:available,submit:submit,status:status,watch:watch,resumeTracking:resumeTracking,cancel:cancel,forget:forget};
