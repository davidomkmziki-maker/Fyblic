(function(){
  ...
  var hideTimer=null,currentJobId='',currentDetail=null,mountTimer=null,lastStoryDetail=null,cancelBusy=false;
  ...
  function activeList(){
    var list=json(ACTIVE_GROUPS_KEY,[]);if(!Array.isArray(list))list=[];
    try{var legacy=json(LEGACY_ACTIVE_KEY,null);if(legacy&&legacy.id&&!list.some(function(x){return x&&String(x.id)===String(legacy.id);}))list.unshift(legacy);localStorage.removeItem(LEGACY_ACTIVE_KEY);}catch(_e){}
    return list.filter(function(x){return x&&x.id;});
  }
  function currentActive(){var list=activeList();return list.length?list[0]:null;}
  function isCurrent(id){var value=currentActive();return !value||!value.id||String(value.id)===String(id||'');}
  function isTracked(id){id=String(id||'');if(!id)return false;return activeList().some(function(x){return x&&String(x.id)===id;});}
  function clearActive(id){id=String(id||'');if(!id)return;try{var list=activeList().filter(function(x){return x&&String(x.id)!==id;});localStorage.setItem(ACTIVE_GROUPS_KEY,JSON.stringify(list.slice(0,24)));}catch(_e){}}
  function forgetOnly(id,jobIds){
    id=String(id||'');var set={};(Array.isArray(jobIds)?jobIds:[]).forEach(function(x){set[String(x)]=1;});
    try{['FYBLIC_PUBLICATION_JOBS_V1080','FYBLIC_PUBLICATION_JOBS_V2'].forEach(function(key){var list=JSON.parse(localStorage.getItem(key)||'[]');if(Array.isArray(list))localStorage.setItem(key,JSON.stringify(list.filter(function(x){return x&&!set[String(x.id||'')]&&String(x.id||'')!==id&&String(x.groupId||'')!==id;})));});}catch(_e){}
    try{var groups=JSON.parse(localStorage.getItem('FYBLIC_PUBLICATION_GROUPS_V1080')||'[]');if(Array.isArray(groups))localStorage.setItem('FYBLIC_PUBLICATION_GROUPS_V1080',JSON.stringify(groups.filter(function(x){return x&&String(x.id||'')!==id;})));}catch(_g){}
  }
  function purgeLocal(detail){
    detail=detail||{};var id=String(detail.groupId||detail.jobId||''),ids=Array.isArray(detail.jobIds)?detail.jobIds:[];
    try{var engine=window.FyblicPublicationEngineV1082;if(engine&&typeof engine.purgeGroup==='function'){engine.purgeGroup(id,ids);return;}}catch(_e){}
    forgetOnly(id,ids);clearActive(id);unmute(id);
  }
  ...
  function closeCancelDialog(){var modal=document.getElementById('fyblicPublicationCancelModalV1117R2');if(modal)modal.classList.remove('show');}
  function ensureCancelDialog(){
    var modal=document.getElementById('fyblicPublicationCancelModalV1117R2');if(modal)return modal;
    var style=document.createElement('style');style.id='fyblicPublicationCancelStyleV1117R2';style.textContent='#fyblicPublicationCancelModalV1117R2{position:fixed;inset:0;z-index:2147483000;display:none;align-items:flex-end;justify-content:center;background:rgba(0,0,0,.58);padding:18px;box-sizing:border-box}#fyblicPublicationCancelModalV1117R2.show{display:flex}.fyblicCancelSheetV1117R2{width:min(520px,100%);background:#14171d;border:1px solid rgba(255,255,255,.12);border-radius:22px;padding:18px;box-shadow:0 18px 70px rgba(0,0,0,.55);font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#fff}.fyblicCancelTitleV1117R2{font-size:18px;font-weight:900;margin:0 0 7px}.fyblicCancelTextV1117R2{font-size:13px;line-height:1.45;color:#b8bfca;margin:0 0 16px}.fyblicCancelActionsV1117R2{display:grid;gap:9px}.fyblicCancelActionsV1117R2 button{min-height:48px;border-radius:14px;font:850 14px system-ui;border:1px solid rgba(255,255,255,.13)}.fyblicCancelKeepV1117R2{background:#20242c;color:#fff}.fyblicCancelConfirmV1117R2{background:#fff;color:#090a0d}.fyblicCancelErrorV1117R2{display:none;color:#ff8d8d;font:750 12px/1.35 system-ui;margin:0 0 10px}.fyblicCancelErrorV1117R2.show{display:block}';document.head.appendChild(style);
    modal=document.createElement('div');modal.id='fyblicPublicationCancelModalV1117R2';modal.innerHTML='<div class="fyblicCancelSheetV1117R2" role="dialog" aria-modal="true" aria-labelledby="fyblicCancelTitleV1117R2"><div id="fyblicCancelTitleV1117R2" class="fyblicCancelTitleV1117R2">Annuler cette publication ?</div><div class="fyblicCancelTextV1117R2">La publication sera arrêtée et retirée de la file. Les autres publications continueront normalement.</div><div class="fyblicCancelErrorV1117R2"></div><div class="fyblicCancelActionsV1117R2"><button type="button" class="fyblicCancelKeepV1117R2">Continuer la publication</button><button type="button" class="fyblicCancelConfirmV1117R2">Annuler la publication</button></div></div>';document.body.appendChild(modal);
    modal.querySelector('.fyblicCancelKeepV1117R2').onclick=function(){if(!cancelBusy)closeCancelDialog();};
    modal.addEventListener('click',function(e){if(e.target===modal&&!cancelBusy)closeCancelDialog();});
    modal.querySelector('.fyblicCancelConfirmV1117R2').onclick=async function(){
      if(cancelBusy||!currentJobId)return;var engine=window.FyblicPublicationEngineV1082,btn=this,errorBox=modal.querySelector('.fyblicCancelErrorV1117R2');
      if(!engine||typeof engine.cancelGroup!=='function'){errorBox.textContent='Service d’annulation indisponible.';errorBox.classList.add('show');return;}
      cancelBusy=true;btn.disabled=true;btn.textContent='Annulation…';errorBox.classList.remove('show');
      try{
        await engine.cancelGroup(currentJobId,currentDetail&&Array.isArray(currentDetail.jobIds)?currentDetail.jobIds:[]);
        closeCancelDialog();
      }catch(error){errorBox.textContent=safeError(error&&error.message||error);errorBox.classList.add('show');}
      finally{cancelBusy=false;btn.disabled=false;btn.textContent='Annuler la publication';}
    };
    return modal;
  }
  function openCancelDialog(){if(!currentJobId||cancelBusy)return;ensureCancelDialog().classList.add('show');}
  ...
  function ensure(){
    ...
    box=document.createElement('div');box.id='fyblicPublicationProgressV2';box.setAttribute('aria-live','polite');box.innerHTML='<div class="fyblicPubV2Card"><button type="button" class="fyblicPubV2Close" aria-label="Annuler la publication">×</button><div class="fyblicPubV2Copy"><span>Publication en cours</span><span>0%</span></div><div class="fyblicPubV2Track"><div class="fyblicPubV2Fill"></div></div><button type="button" class="fyblicPubV2Retry">Réessayer depuis Publications</button></div>';
    ...
    box.querySelector('.fyblicPubV2Close').onclick=function(){clearTimeout(hideTimer);openCancelDialog();};
  ...
  function render(detail){
    detail=detail||{};var box=ensure(),jobId=String(detail.jobId||'');mount(box);if(jobId&&isCurrent(jobId))currentDetail=detail;
    if(String(detail.publicationType||'')==='story'){
      var currentStory=isCurrent(jobId);
      if(currentStory&&jobId)currentJobId=jobId;
      if(currentStory)box.classList.remove('show');
      if(detail.status==='published'){addProfileNotice(detail,'Ta Story est entièrement prête.','success');try{localStorage.setItem('HAPPYAD_RADAR_REFRESH_NEEDED','1');}catch(_e){}clearActive(jobId);if(currentStory)clearStoryRing();refreshHome();return;}
      if(detail.status==='canceled'){purgeLocal(detail);if(currentStory)clearStoryRing();refreshHome();return;}
      if(detail.primaryReady===true){try{localStorage.setItem('HAPPYAD_RADAR_REFRESH_NEEDED','1');}catch(_e){}clearActive(jobId);if(currentStory)clearStoryRing();refreshHome();return;}
      if(!currentStory)return;
      paintStoryRing(detail);
      if(detail.status==='failed'){purgeLocal(detail);addProfileNotice(detail,safeError(detail.error),'warning');if(currentStory)setTimeout(clearStoryRing,1400);return;}
      return;
    }
    /* V1117 : plusieurs publications peuvent vivre en même temps. La barre
       représente seulement la plus récente; les anciennes continuent en silence
       mais leurs fins/erreurs sont toujours nettoyées. */
    var currentNormal=isCurrent(jobId);
    if(currentNormal&&jobId)currentJobId=jobId;
    if(!currentNormal&&detail.status!=='published'&&detail.status!=='canceled'&&detail.status!=='failed'&&detail.primaryReady!==true)return;
    if(String(detail.publicationType||'')==='boutique'&&detail.status==='published'){
      unmute(jobId);clearActive(jobId);forgetOnly(jobId);if(currentNormal)box.classList.remove('show');return;
    }
    if(detail.status==='published'){
      addProfileNotice(detail,'Toutes les qualités de ta publication sont prêtes.','success');
      unmute(jobId);clearActive(jobId);forgetOnly(jobId);refreshHome();if(currentNormal)box.classList.remove('show');return;
    }
    if(detail.primaryReady===true){mute(jobId);clearActive(jobId);refreshHome();if(currentNormal)box.classList.remove('show');return;}
    if(detail.status==='canceled'){purgeLocal(detail);if(currentNormal)box.classList.remove('show');return;}
    if(isMuted(jobId)){
      ...
      if(currentNormal)box.classList.remove('show');return;
    }
    ...
    if(detail.status==='failed'){purgeLocal(detail);if(!currentNormal){addProfileNotice(detail,safeError(detail.error),'warning');return;}card.classList.add('fail');copy.children[0].textContent=safeError(detail.error);copy.children[1].textContent='Échec';hideTimer=setTimeout(function(){box.classList.remove('show');},1400);}
  }
  ...
  function recoverActiveLineV1117R2(){var active=currentActive();if(active&&active.id)unmute(String(active.id));}
  window.addEventListener('pageshow',function(){recoverActiveLineV1117R2();var box=ensure();mount(box);},{passive:true});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){recoverActiveLineV1117R2();ensure();},{once:true});else{recoverActiveLineV1117R2();ensure();}
})();
