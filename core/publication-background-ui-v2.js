(function(){
  ...
  var hideTimer=null;
  function safeError(value){var m=String(value||'');return /supabase|23502|failing row|violates|constraint|postgres|pgrst|sql/i.test(m)?'Publication impossible. Réessaie après la mise à jour.':(m.slice(0,140)||'Publication impossible. Réessaie.');}
  function ensure(){
  ...
  function render(detail){
    ...
    if(detail.primaryReady===true||detail.status==='published'){card.classList.add('ok');copy.children[0].textContent=detail.status==='published'?'Publication publiée':'Publication affichée · optimisation en arrière-plan';fill.style.width='100%';copy.children[1].textContent='100%';try{localStorage.setItem('HAPPYAD_HOME_REFRESH_NEEDED','1');sessionStorage.removeItem('HAPPYAD_HOME_POSTS_LAST_SYNC');if(typeof window.happyadRefreshHomePostsNow==='function')window.happyadRefreshHomePostsNow('pipeline-v2-primary-ready');}catch(_e){}hideTimer=setTimeout(function(){box.classList.remove('show');},5000);}
    else if(detail.status==='failed'){card.classList.add('fail');copy.children[0].textContent=safeError(detail.error);copy.children[1].textContent='Échec';}
