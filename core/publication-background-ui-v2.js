(function(){
  ...
  function ownStoryAvatar(){
    var root=radarAnchor();if(!root)return null;
    var add=root.querySelector('.haStoryAddOnlyV629 .radarAvatar');if(add)return add;
    var uid='';try{var u=JSON.parse(localStorage.getItem('HAPPYAD_CENTRAL_USER_V10_CLEAN_STATS_FULL')||'{}');uid=String(u.id||u.user_id||'');}catch(_e){}
    if(uid){var safe=window.CSS&&CSS.escape?CSS.escape(uid):uid.replace(/"/g,'\\"'),owned=root.querySelector('button.radarItem[data-story-owner="'+safe+'"] .radarAvatar');if(owned)return owned;}
    return root.querySelector('.haStoryRadarUnitV629 .radarAvatar');
  }
  function clearStoryRing(){document.querySelectorAll('.fyblicStoryProgressV1068').forEach(function(node){node.remove();});lastStoryDetail=null;}
  function paintStoryRing(detail){
    lastStoryDetail=detail;var avatar=ownStoryAvatar();if(!avatar)return;
    document.querySelectorAll('.fyblicStoryProgressV1068').forEach(function(node){if(node.parentNode!==avatar)node.remove();});
    var ring=avatar.querySelector('.fyblicStoryProgressV1068');if(!ring){ring=document.createElement('span');ring.className='fyblicStoryProgressV1068';ring.setAttribute('aria-hidden','true');avatar.appendChild(ring);}
    var p=Math.max(1,Math.min(100,Number(detail&&detail.progress||1)));ring.style.setProperty('--fyblic-story-angle',(p*3.6)+'deg');ring.classList.toggle('fail',detail&&detail.status==='failed');ring.title=(detail&&detail.stage||'Story en cours')+' — '+p+'%';
  }
  function mount(box){
  ...
  function render(detail){
    ...
    if(String(detail.publicationType||'normal')==='story'){
      box.classList.remove('show');
      if(detail.status==='published'||detail.primaryReady===true){clearStoryRing();refreshHome();return;}
      if(detail.status==='canceled'){clearStoryRing();return;}
      paintStoryRing(detail);
      if(detail.status==='failed'){addProfileNotice(detail,'La Story n’a pas pu être terminée.','warning');setTimeout(clearStoryRing,7000);}
      return;
    }
