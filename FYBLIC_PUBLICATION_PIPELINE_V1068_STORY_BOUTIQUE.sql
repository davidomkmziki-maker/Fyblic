-- FYBLIC V1068 — activation définitive Story + Boutique sur le pipeline durable.
-- Pré-requis : V1063 puis V1064 déjà exécutés.
-- Exécuter dans Supabase SQL Editor AVANT le worker V1068.
...
begin
  ...
  if v_job.publication_type not in ('normal','story','boutique') then raise exception 'TYPE_NOT_ENABLED_V1068'; end if;
  ...
  if v_job.publication_type='normal' then
    ...
    v_story_id:=public.fyblic_insert_story_json_v1068(v_story);
...
select publication_type,count(*) from public.fyblic_publication_jobs group by publication_type order by publication_type;
